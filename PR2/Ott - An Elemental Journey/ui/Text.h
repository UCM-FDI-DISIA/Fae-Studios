...
public:
    ...
    Text(const std::string& text, const Font& f, const Vector2D& position, SDL_Renderer* renderer, const SDL_Color& c = {255,255,255}, const Scale& scale = Scale(1.0f, 1.0f)) :
            UIElement(position, new Texture(renderer, text, f, c), scale) {};

    ~Text() override { delete texture; }
