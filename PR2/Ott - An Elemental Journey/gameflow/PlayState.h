...
public:
	PlayState(int stateID, SDLApplication* app) : GameState(stateID,app) {
		//enemy = static_cast<MeleeEnemy*> (addObject(new MeleeEnemy(Vector2D(300, 450), app->getTexture("enemy", stateID), 5, elementsInfo::Earth, nullptr, false, Vector2D(1, 0), Scale(0.5, 0.5), 110, 100, this)));
		Ground* ground;
		enemy = static_cast<MeleeEnemy*> (addObject(new MeleeEnemy(Vector2D(200,420), app->getTexture("bug", stateID), 5, elementsInfo::Earth, nullptr, false, Vector2D(1, 0), Scale(2.5, 2.5), 5, 100, this, app->getTexture("whiteBox",stateID))));
		physicObjects.push_back(enemy);
		enemy = static_cast<MeleeEnemy*> (addObject(new MeleeEnemy(Vector2D(300,100), app->getTexture("bug", stateID), 5, elementsInfo::Earth, enemy, true, Vector2D(1, 0), Scale(2.5, 2.5), 5, 100, this, app->getTexture("whiteBox", stateID))));
		physicObjects.push_back(enemy);
