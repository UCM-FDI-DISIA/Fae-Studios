...
public:
	PlayState(int stateID, SDLApplication* app) : GameState(stateID,app) {
		...
		enemy = static_cast<MeleeEnemy*> (addObject(new MeleeEnemy(Vector2D(300, 100), app->getTexture("enemy", stateID), 5, elementsInfo::Earth, nullptr, true, Vector2D(1, 0), Scale(0.5, 0.5), 110, this)));
