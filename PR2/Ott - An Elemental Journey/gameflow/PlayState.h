...
public:
	PlayState(int stateID, SDLApplication* app) : GameState(stateID,app) {
		...
		enemy = static_cast<SlimeEnemy*> (addObject(new SlimeEnemy(3, Vector2D(270, 400), app->getTexture("slime", stateID), 5, elementsInfo::Earth, enemy, true, Vector2D(1, 0), Scale(3.0, 3.0),110, 100, this)));
		physicObjects.push_back(enemy);
