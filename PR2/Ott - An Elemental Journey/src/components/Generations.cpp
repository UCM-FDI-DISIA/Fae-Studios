...
void Generations::Divide() {
	...
	for (int i = 0; i < 2; i++) {
		...
		constructors::eSlime(mngr_, ent_->getComponent<FramedImage>()->getTexture(), (int)pos.getX(), (int)pos.getY(), scale, getGeneration() - 1, ent_->getComponent<Health>()->getMaxHealth() / 2 - 1);
