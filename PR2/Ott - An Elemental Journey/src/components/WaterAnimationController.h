...
enum WaterAnims { WATER_ADVANCE, WATER_BACK };

class WaterAnimationController : public Component
{
private:
...
public:
	constexpr static ecs::cmpId_type id = ecs::_WATER_ATTACK_ANIM;

	WaterAnimationController(anims::Entities e) : eAnims(e)
	{
		timer_ = 0;
		startingCol = 0;
	};
	virtual ~WaterAnimationController() {};
	void initComponent();
	virtual void update();
