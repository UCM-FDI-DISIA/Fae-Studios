...
{
	if (cameraShake) {
		int muyBuenas = SDL_GetTicks() - holi;
		if( left && muyBuenas <= 50) {
			camera.x+= 10;
			
		}
		else if (left && muyBuenas > 50) {
			left = false;
			right = true;
			holi = SDL_GetTicks();
		}
		muyBuenas = SDL_GetTicks() - holi;

		if (right && muyBuenas <= 50) {
			camera.x-=10;
		}
		else if (right && muyBuenas > 50) {
			left = true;
			right = false;
			holi = SDL_GetTicks();
		}
